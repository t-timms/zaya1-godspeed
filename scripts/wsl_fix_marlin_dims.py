...
while i < len(lines):
    ...
    if line.strip() == 'def apply_weights(':
        ...
        new_lines.append('                from compressed_tensors.compressors.nvfp4.helpers import unpack_fp4_from_uint8')
        ...
        new_lines.append('                w = dequantize(x_q=w, scale=ws.float(), global_scale=wgs, dtype=ws.float().dtype)')
